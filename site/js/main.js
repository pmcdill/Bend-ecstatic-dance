(() => {
  ...
  // ---------- "Add to calendar" (recurring weekly event, not tied to any
  // specific artist booking — generates a .ics file client-side since this
  // site has no backend) ----------
  ...
  function icsDate(date, hours, minutes) {
    return date.getFullYear() + pad2(date.getMonth() + 1) + pad2(date.getDate()) + 'T' + pad2(hours) + pad2(minutes) + '00';
  }

  // DTSTAMP must be a true UTC timestamp — unlike icsDate() above (which
  // builds a local wall-clock time for the TZID-qualified DTSTART/DTEND),
  // every component here has to come from the UTC getters or the date and
  // time portions can disagree across a UTC day boundary.
  function icsUTCStamp(date) {
    return date.getUTCFullYear() + pad2(date.getUTCMonth() + 1) + pad2(date.getUTCDate()) +
      'T' + pad2(date.getUTCHours()) + pad2(date.getUTCMinutes()) + pad2(date.getUTCSeconds()) + 'Z';
  }

  // RFC 5545 requires escaping commas/semicolons/backslashes and folding
  // lines over 75 octets — this content never contains the former, so only
  ...
  function buildWeeklyEventICS() {
    const start = nextThursdayOnOrAfter(new Date());
    const description = 'A substance-free, barefoot, freeform dance floor. ' +
      (VENUE_DETAILS.when || 'Every Thursday · Doors 6:45pm · Dance 8-10pm') + '. ' +
      (VENUE_DETAILS.price || 'Sliding scale: $18-22') + '.';

    const lines = [
      'BEGIN:VCALENDAR',
      'VERSION:2.0',
      'PRODID:-//Bend Ecstatic Dance//bendecstaticdance.com//EN',
      'CALSCALE:GREGORIAN',
      'BEGIN:VEVENT',
      'UID:weekly-dance@bendecstaticdance.com',
      'DTSTAMP:' + icsUTCStamp(new Date()),
      'DTSTART;TZID=America/Los_Angeles:' + icsDate(start, 18, 45),
      'DTEND;TZID=America/Los_Angeles:' + icsDate(start, 22, 0),
      'RRULE:FREQ=WEEKLY;BYDAY=TH',
      'SUMMARY:Bend Ecstatic Dance',
      'DESCRIPTION:' + description,
      'LOCATION:The Glow Pad at Tula Movement Arts\\, 2797 NW Clearwater Dr\\, Bend\\, OR',
      'URL:https://www.bendecstaticdance.com/',
      'END:VEVENT',
      'END:VCALENDAR',
    ];
  ...
  if (addToCalendarBtn) {
    addToCalendarBtn.addEventListener('click', () => {
      const blob = new Blob([buildWeeklyEventICS()], { type: 'text/calendar;charset=utf-8' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'bend-ecstatic-dance-weekly.ics';
